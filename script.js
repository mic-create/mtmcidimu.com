...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mediaPanel = document.getElementById('heroMediaPanel');
    ...
    // Set total counter display
    if (totalNumEl) {
        totalNumEl.textContent = totalSlides < 10 ? `0${totalSlides}` : totalSlides;
    }
    ...
    function showSlide(index) {
        ...
        if (currentNumEl) {
            currentNumEl.textContent = (index + 1) < 10 ? `0${index + 1}` : (index + 1);
        }
    ...
    if (mediaPanel) {
        mediaPanel.addEventListener('mouseenter', stopAutoPlay);
        mediaPanel.addEventListener('mouseleave', startAutoPlay);
    }
