...
/**
 * Create a new appointment (Preserved)
 * POST /api/appointments
 */
export const createAppointment = async (req, res, next) => {
  try {
    ...
    if (email) {
      try {
        ...
        if (existingPatient) {
          ...
          const newPatient = await prisma.patient.create({
            data: { firstName: fName, lastName: lName, email, phone }
          });
          patientId = newPatient.id;
        }
      } catch (pErr) {
        console.warn('Patient lookup/creation warning:', pErr.message);
      }
    }
...
export const updateAppointmentStatus = async (req, res, next) => {
  try {
    ...
    if (!validStatuses.includes(normalizedStatus)) {
      return errorResponse(
        res,
        `Invalid status '${status}'. Valid status options are: ${validStatuses.join(', ')}`,
        400
      );
    }
    ...
    if (!existingAppointment) {
      return errorResponse(res, `Appointment with ID '${id}' not found.`, 404);
    }
    ...
    console.log(`[Admin] Successfully updated appointment ${id} status to ${normalizedStatus}`);

    return successResponse(
      res,
      `Appointment status updated to ${normalizedStatus} successfully.`,
      updatedAppointment
    );

  } catch (error) {
    console.error('CRITICAL: Error updating appointment status:', error);
    return errorResponse(
      res,
